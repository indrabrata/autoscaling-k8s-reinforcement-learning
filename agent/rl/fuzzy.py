...
class Fuzzy:
    ...
    def apply_rules(self, fz: Dict[str, Dict[str, float]]) -> Dict[str, float]:
        ...
        req_rate = fz.get("request_rate_normalized", {})

        scale_up = max(
            # High response time dominates
            max(resp.get("high", 0.0), resp.get("very_high", 0.0)) * 1.0,
            # CPU or Memory high and response medium/high
            min(
                max(cpu.get("high", 0.0), mem.get("high", 0.0)),
                max(resp.get("medium", 0.0), resp.get("high", 0.0)),
            )
            * 0.9,
            ...
            min(cpu.get("medium", 0.0), mem.get("medium", 0.0), resp.get("high", 0.0))
            * 0.85,
            # CPU or Mem high while response is deteriorating (medium)
            max(cpu.get("high", 0.0), mem.get("high", 0.0))
            * resp.get("medium", 0.0)
            * 0.8,
            # Request rate saturating capacity (NEW)
            max(req_rate.get("very_high", 0.0), req_rate.get("high", 0.0)) * 0.95,
            # Request rate high + CPU/MEM medium (approaching limits - NEW)
            min(
                req_rate.get("high", 0.0),
                max(cpu.get("medium", 0.0), mem.get("medium", 0.0)),
            )
            * 0.85,
            # Request rate very high with high response time (critical - NEW)
            min(
                req_rate.get("very_high", 0.0),
                max(resp.get("high", 0.0), resp.get("very_high", 0.0)),
            )
            * 1.0,
        )
        ...
        scale_down = max(
            ...
            min(cpu.get("low", 0.0), mem.get("low", 0.0), resp.get("medium", 0.0))
            * 0.9,
            ...
            * 0.8,
            # Request rate very low (under-utilized capacity - NEW)
            req_rate.get("very_low", 0.0) * 0.9,
            # Request rate low with low CPU/MEM (wasteful - NEW)
            min(req_rate.get("low", 0.0), max(cpu.get("low", 0.0), mem.get("low", 0.0)))
            * 0.85,
        )

        no_change = max(
            # Balanced load
            min(cpu.get("medium", 0.0), mem.get("medium", 0.0), resp.get("medium", 0.0))
            * 1.0,
            ...
            min(cpu.get("medium", 0.0), mem.get("medium", 0.0), resp.get("low", 0.0))
            * 0.9,
            # CPU low, mem medium, response medium — stable
            min(cpu.get("low", 0.0), mem.get("medium", 0.0), resp.get("medium", 0.0))
            * 0.85,
            ...
            * 0.8,
            # Request rate medium with balanced resources (optimal utilization - NEW)
            min(
                req_rate.get("medium", 0.0),
                max(cpu.get("medium", 0.0), mem.get("medium", 0.0)),
                resp.get("low", 0.0),
            )
            * 0.95,
    ...
    def apply_reward_rules(self, fz: Dict[str, Dict[str, float]]) -> Dict[str, float]:
        ...
        wasteful_state = max(
            ...
            * 1.0,
            ...
            min(cpu.get("low", 0.0), mem.get("low", 0.0), resp.get("very_low", 0.0))
            * 0.9,
            ...
            * 0.8,
            ...
            min(cpu.get("low", 0.0), mem.get("low", 0.0), resp.get("low", 0.0)) * 0.7,
        )
